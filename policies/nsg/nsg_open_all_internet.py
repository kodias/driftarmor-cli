...
from nsg_common import (  # type: ignore[import-not-found]
    ...
    opens_all_ports_from_internet,
)


class NsgOpenAllInternet(BaseResourceCheck):
    def __init__(self) -> None:
        super().__init__(
            name="NSG must not allow all ports (*) from the Internet",
            id="CKV_DRIFTARMOR_NSG_3",
    ...
    def scan_resource_conf(self, conf):  # type: ignore[no-untyped-def]
        if "direction" in conf and "access" in conf:
            if opens_all_ports_from_internet(conf):
                self.evaluated_keys = [
                    "direction",
                    "access",
        ...
        for rule in iter_nsg_inline_rules(conf):
            if opens_all_ports_from_internet(rule):
