...
class NsgOpenSshInternet(BaseResourceCheck):
    def __init__(self) -> None:
        super().__init__(
            name="NSG must not allow SSH (22) from the Internet",
            id="CKV_DRIFTARMOR_NSG_1",
    ...
    def scan_resource_conf(self, conf):  # type: ignore[no-untyped-def]
        if "direction" in conf and "access" in conf:
            if opens_port_from_internet(conf, "22"):
                self.evaluated_keys = [
                    "direction",
                    "access",
        ...
        for rule in iter_nsg_inline_rules(conf):
            if opens_port_from_internet(rule, "22"):
